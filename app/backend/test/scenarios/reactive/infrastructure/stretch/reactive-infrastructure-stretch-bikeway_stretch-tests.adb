...
package body Reactive.Infrastructure.Stretch.Bikeway_Stretch.Tests is
   ...
   -----------------------------------------------------
   Street_Id : Infra_Id;
   Bikeway_Stretch_Id : Infra_Id;
   Bus_Id : Natural;
   Stretch_Size : Natural;

   procedure Set_Up (T: in out Bikeway_Stretch_Test) is
   begin
      Street_Id := 1;
      Bikeway_Stretch_Id := 2;
      Bus_Id := 10;
      Stretch_Size := 4;
      ...
      T.Stretch
         := Stretch.Reference (
            Bikeway_Stretch.Create (
               Id              => Bikeway_Stretch_Id,
               Size            => Stretch_Size,
   ...
   --                  REGISTRATION
   -----------------------------------------------------
   procedure Register_Tests (T: in out Bikeway_Stretch_Test) is
   begin
      Stretch.Tests.Register_Tests (Stretch.Tests.Stretch_Test (T));
   end Register_Tests;

   function Name(T: Bikeway_Stretch_Test) return AU.Message_String is
   begin
      return AU.Format ("Bikeway_Stretch");
   end Name;
