...
package body Reactive.Infrastructure.Factory.Street_Factory.Bikeway_Factory.Tests is
   package Ass renames AUnit.Assertions;

   -----------------------------------------------------
   --                   SCAFFOLDING
   -----------------------------------------------------
   Factory_Ref : Reactive.Infrastructure.Factory.Street_Factory.Bikeway_Factory.Reference;

   procedure Set_Up (T : in out Bikeway_Factory_Test) is
   begin
      Factory_Ref := new Bikeway_Factory.Object;
   end Set_Up;

   -- Test Routines:
   procedure Test_Create_Bikeway_Stretch (T: in out TC.Test_Case'Class) is
   ...
   end Test_Create_Bikeway_Stretch;

   procedure Test_Create_Bikeway_Lane (T: in out TC.Test_Case'Class) is
      Lane_Id        : Infra_Id := 42;
   ...
   end Test_Create_Bikeway_Lane;

   procedure Register_Tests (T : in out Bikeway_Factory_Test) is
      use TC.Registration;
   begin
      Register_Routine (Test    => T,
                        Routine => Test_Create_Bikeway_Stretch'Access,
                        Name    => "Test stretch creation");

      Register_Routine (Test    => T,
                        Routine => Test_Create_Bikeway_Lane'Access,
                        Name    => "Test lane creation");
   end Register_Tests;

   function Name (T : in Bikeway_Factory_Test)
      return AU.Message_String is
   begin
      return AU.Format ("Bikeway_Factory");
   end Name;
