...
package body Reactive.Infrastructure.Factory.Street_Factory.Footway_Factory.Tests is
   package Ass renames AUnit.Assertions;

   -----------------------------------------------------
   --                   SCAFFOLDING
   -----------------------------------------------------
   Factory_Ref : Reactive.Infrastructure.Factory.Street_Factory.Footway_Factory.Reference;

   procedure Set_Up (T : in out Footway_Factory_Test) is
   begin
      Factory_Ref := new Footway_Factory.Object;
   end Set_Up;

   -- Test Routines:
   procedure Test_Create_Footway_Stretch (T: in out TC.Test_Case'Class) is
   ...
   end Test_Create_Footway_Stretch;

   procedure Test_Create_Footway_Lane (T: in out TC.Test_Case'Class) is
      Lane_Id        : Infra_Id := 42;
   ...
   end Test_Create_Footway_Lane;

   procedure Register_Tests (T : in out Footway_Factory_Test) is
      use TC.Registration;
   begin
      Register_Routine (Test    => T,
                        Routine => Test_Create_Footway_Stretch'Access,
                        Name    => "Test stretch creation");

      Register_Routine (Test    => T,
                        Routine => Test_Create_Footway_Lane'Access,
                        Name    => "Test lane creation");
   end Register_Tests;

   function Name (T : in Footway_Factory_Test)
      return AU.Message_String is
   begin
      return AU.Format ("Footway_Factory");
   end Name;
